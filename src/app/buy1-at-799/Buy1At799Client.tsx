...
export default function Buy1At799Client() {
  ...
  const handleFreeSelect = (product: ExtendedProduct) => {
    setSelectedFree(product);
    setAddedToCart(false);
  };

  const handleAddToCart = () => {
    if (!selectedMain || !selectedFree) return;

    const bundleProduct = {
      id: Date.now(),
      name: `Buy 1 Get 1: ${selectedMain.name.split(' ')[0]} + FREE ${selectedFree.name.split(' ')[0]}`,
  ...
  const isComplete = selectedMain && selectedFree;
            ...
            <div className="relative">
              <div className="w-48 h-48 md:w-64 md:h-64 bg-white/20 backdrop-blur-sm rounded-full flex items-center justify-center border-4 border-white/30">
                <div className="text-center">
                  ...
                  <p className="text-white font-bold text-sm md:text-base mt-2">+ FREE GIFT</p>
                </div>
              </div>
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="bg-white rounded-2xl shadow-lg p-6 mb-8">
          <div className="flex items-center justify-between max-w-2xl mx-auto">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="hidden md:block">
                <p className="font-semibold text-gray-800">Main Perfume</p>
            ...
            <div className="flex items-center gap-3">
              <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold ${
                selectedFree ? 'bg-teal-500 text-white' : step === 2 ? 'bg-teal-500 text-white' : 'bg-gray-200 text-gray-500'
              }`}>
                {selectedFree ? <Check className="w-5 h-5" /> : '2'}
              </div>
              <div className="hidden md:block">
                <p className="font-semibold text-gray-800">FREE Gift</p>
                <p className="text-xs text-gray-500">10ml Travel Size</p>
              </div>
            </div>
          </div>
          ...
          <div className="mt-4 pt-4 border-t flex flex-wrap items-center justify-center gap-2 text-sm">
            <span className={selectedMain ? 'text-teal-600 font-medium' : 'text-gray-400'}>
              {selectedMain ? `✓ ${selectedMain.name.split(' ').slice(0, 2).join(' ')}` : '○ Select main perfume'}
            </span>
            <span className="text-gray-300">+</span>
            <span className={selectedFree ? 'text-teal-600 font-medium' : 'text-gray-400'}>
              {selectedFree ? `✓ FREE ${selectedFree.name.split(' ').slice(0, 2).join(' ')}` : '○ Select free gift'}
            </span>
          </div>
        </div>

        {/* Step 1: Main Perfume Selection */}
        {step === 1 && !selectedMain && (
          <div>
            <h2 className="text-2xl font-bold text-gray-800 mb-6 flex items-center gap-2">
              <Sparkles className="w-6 h-6 text-teal-500" />
              Step 1: Choose Your Main Perfume (100ml)
            </h2>
        ...
        {step === 2 && selectedMain && !isComplete && (
          <div>
            ...
            <div className="bg-teal-50 rounded-xl p-4 mb-6 flex items-center gap-4">
              ...
              <div>
                <p className="text-xs text-teal-600 font-semibold">MAIN PERFUME</p>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
              {miniPerfumes.map((product) => (
                <div
                  key={product.id}
                  onClick={() => handleFreeSelect(product)}
                  className="bg-white rounded-xl shadow-md overflow-hidden cursor-pointer transform transition-all duration-300 hover:scale-105 hover:shadow-xl border-2 border-transparent hover:border-teal-400"
                >
                  <div className="relative aspect-square bg-gray-100">
                    {product.images && product.images[0] && (
              ...
              <div className="p-6">
                ...
                <div className="flex items-center gap-4 p-4 bg-teal-50 rounded-xl mb-4">
                  ...
                  <div className="flex-1">
                    <p className="text-xs text-teal-600 font-semibold mb-1">MAIN PERFUME</p>
                ...
                <div className="flex items-center gap-4 p-4 bg-green-50 rounded-xl mb-6">
                  <div className="w-20 h-20 rounded-lg overflow-hidden flex-shrink-0 bg-gray-100">
                    {selectedFree.images && selectedFree.images[0] && (
                      <img
                        src={selectedFree.images[0].src}
                        alt={selectedFree.name}
                        className="w-full h-full object-cover"
                      />
                    )}
                  </div>
                  <div className="flex-1">
                    <p className="text-xs text-green-600 font-semibold mb-1">FREE GIFT</p>
                    <h3 className="font-semibold text-gray-800">{selectedFree.name}</h3>
                    <p className="text-sm text-gray-500">10ml Travel Size</p>
                  </div>
                ...
                <div className="border-t pt-4">
                  <div className="flex justify-between items-center mb-2">
                    <span className="text-gray-600">Main Perfume (100ml)</span>
                    <span className="text-gray-600">₹{selectedMain.price}</span>
                  </div>
                  <div className="flex justify-between items-center mb-2">
                    <span className="text-gray-600">Free Gift (10ml)</span>
        ...
        <div className="mt-12 grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            { icon: '🎁', title: 'FREE Gift', desc: '10ml travel size included' },
